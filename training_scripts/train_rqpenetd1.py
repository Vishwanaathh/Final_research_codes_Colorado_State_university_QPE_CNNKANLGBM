...
train_rqpenetd1.py
==================
Training script for RQPENetD1 — DenseNet-based QPE model.

Trains on the same T_{4x9x9} dataset used by CNN+KAN, enabling
direct comparison between architectures.

Pipeline
--------
    (N, 4, 9, 9) --[RQPENetD1]--> (N, 1)  log1p(R) prediction

Loss
----
    MSE on log1p(R) targets (consistent with paper: equation 1)
    + L2 weight decay via AdamW

Optimiser
---------
    Adam lr=1e-3 (paper: equation 2)
    CosineAnnealingLR
...
--------
    ...
    Augmentation: rot90 + flip + Z noise (same as CNN+KAN)
...
-------------------
    QPE_CNN_KAN_LIGHTGBM/
        AI_Script/
            rqpenetd1.py
        data/dataset/processed_T4/
        models/
            rqpenetd1/               <- outputs here
                best_model.pt
                training_history.json
                training_log.txt
        training_scripts/
            train_rqpenetd1.py       <- THIS FILE

Usage
-----
    python train_rqpenetd1.py
...
try:
    ...
    sys.exit(f"[FATAL] Cannot import rqpenetd1.py from {_AI_SCRIPT_DIR}\n{e}")
...
def setup_logging(output_dir: Path) -> logging.Logger:
    log = logging.getLogger("rqpenetd1_train")
...
class RadarAugment:
    """Spatial augmentation for T_{4x9x9} tensors."""
...
def train(data_dir: Path, output_dir: Path, epochs: int, batch_size: int,
          lr: float, patience: int, seed: int, log: logging.Logger) -> None:
    ...
    log.info("Val  : %s  y=[%.2f, %.2f] mm/h",
             X_val.shape, y_val.min(), y_val.max())
    ...
    # ── Model ──────────────────────────────────────────────────────────────
    model = RQPENetD1().to(device)
    log.info("RQPENetD1 parameters: %d", count_parameters(model))
    log.info("Block config: (6,12,12,8)  growth_rate=12  stem=32")
    ...
    # MSE loss — consistent with paper (equation 1)
    criterion = nn.MSELoss()

    log.info("Optimiser : Adam  lr=%.4f  weight_decay=1e-4", lr)
    log.info("Loss      : MSELoss (paper equation 1)")
...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        description="Train RQPENetD1",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    p.add_argument("--data_dir",   type=Path, default=_root/"data"/"dataset"/"processed_T4_improved")
    p.add_argument("--output_dir", type=Path, default=_root/"models"/"rqpenetd1")
...
def main():
    ...
    log.info("RQPENetD1 Training started.")
    log.info("Config: %s", vars(args))
    train(data_dir=args.data_dir, output_dir=args.output_dir,
          epochs=args.epochs, batch_size=args.batch_size,
          lr=args.lr, patience=args.patience, seed=args.seed, log=log)
