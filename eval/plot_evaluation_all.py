# -*- coding: utf-8 -*-
"""
plot_evaluation_all.py  (updated)
==================================
Added: plot_continuous_by_category() — grouped bar chart showing
RMSE, MAE, and Bias broken down by rainfall category for all models.

New plot: 02b_continuous_by_category.png
"""
...
MODEL_COLORS  = ["#e74c3c","#e67e22","#f1c40f","#3498db","#2ecc71","#9b59b6","#1abc9c"]
MODEL_MARKERS = ["o", "s", "^", "D", "*", "P", "X"]
CENTER = 4

# Rainfall categories — must match evaluate_all.py
RAIN_CATEGORIES = {
    "light\n(1-10)":     (1.0,   10.0),
    "moderate\n(10-25)": (10.0,  25.0),
    "heavy\n(25-50)":    (25.0,  50.0),
...
def build_lgb_features(X_raw, X_norm, pred_mmh):
    ...
    return np.column_stack([
        ...
        Z_high_std, Z_high_max, ZDR_high_std,
...
def load_predictions(data_dir, models_dir, device):
    ...
    if _HAS_CNNKAN and ckpt is not None:
        ...
        kan_mmh, _ = dl_predict(model_ck, X_val_norm, device)
        ...
        if lgb_path.exists():
            ...
            preds["CNN+KAN+LightGBM"] = apply_lgb_correction(kan_mmh, epsilon_log)
            print("  \u2713 CNN+KAN+LightGBM")
        else:
            print("  \u2717 CNN+KAN+LightGBM \u2014 lgb_residual.pkl not found")
    else:
    ...
    if _HAS_RQPENET and rq_path.exists():
        ...
        rq_mmh, _ = dl_predict(model_rq, normalise(X_val_raw, rq_means, rq_stds), device)
        preds["RQPENetD1"] = rq_mmh
        print("  \u2713 RQPENetD1")
    else:
    ...
    if _HAS_CONVLSTM and cl_path.exists():
        ...
        cl_mmh, _ = dl_predict(model_cl, normalise(X_val_raw, cl_means, cl_stds), device)
        preds["ConvLSTMQPE"] = cl_mmh
        print("  \u2713 ConvLSTMQPE")
    else:
...
def plot_scatter(preds, y_true, out):
    ...
    for i, (name, pred) in enumerate(preds.items()):
        ...
        ax.set_title(
            f"{name}\nRMSE={rmse:.2f}  R\u00b2={r2:.3f}  Bias={bias:+.2f}  r={corr:.3f}",
            fontsize=10)
...
def plot_continuous_bars(preds, y_true, out):
    """Overall RMSE/MAE/Bias bar chart (unchanged)."""
    names  = list(preds.keys())
...
def plot_continuous_by_category(preds, y_true, out):
    """
    One figure per rainfall category (mirrors how categorical plots work).
    Each figure: 1 row x 3 cols (RMSE, MAE, Bias), one bar per model.
    Saves: 02b_continuous_light.png, 02b_continuous_moderate.png, etc.
    """
    names  = list(preds.keys())
    x      = np.arange(len(names))
    w      = 0.55
    ...
    # Pre-compute metrics per model per category
    cat_metrics = {}
    for name, pred in preds.items():
        cat_metrics[name] = {}
    ...
    for cat_label, filename in cat_filenames.items():
        ...
        title_cat = cat_label.replace("\n", " ")

        fig, axes = plt.subplots(1, 3, figsize=(20, 7))
        fig.suptitle(
            f"Continuous Metrics \u2014 {title_cat} mm/h  (N={n_obs:,})",
            fontsize=13, y=1.02)
        ...
        for ax, (metric, ylabel, has_zero, better) in zip(axes, [
            ...
            vals = [cat_metrics[name][cat_label].get(metric) for name in names]
            # Replace None with 0 for plotting, mark separately
            ...
            if finite:
                if better == "zero":
                    ...
                    best_i = min(finite, key=lambda t: t[1])[0]
                bars[best_i].set_edgecolor("black")
                bars[best_i].set_linewidth(2.5)
...
def main():
    ...
    plot_continuous_by_category(preds, y_val, out_dir)   # NEW
